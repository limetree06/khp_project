...
if (window.ethereum) {
  ...
  var contractAddress ='0x7c91baF1C69dE946a4D68e4296845a07478B9232'; 
...
function PrintnAlarm(events, theevent){
  ...
  $("#button_drum").click(function() {
    ...
    Contract.methods.registerDrum(NUM,$("#drumId").val(),waste, $("#temp").val()).send({from: web3.eth.defaultAccount, gas:3000000}).on('transactionHash', function(hash){
    ...
    $("#button_sen").click(function() {
        ...
        if(waste == $("#sensorType option:selected").val() ){
          Contract.methods.registerSensor($("#drumNumsen").val(),$("#sensorId").val(),$("#sensorType option:selected").val()).send({from: web3.eth.defaultAccount, gas:3000000})
    ...
    $("#button_check").click(function(){
      ...
      $("#button_sto").click(function() {
        STATE = "storage";
        NUM = $("#getst").val();
        Contract.methods.storageDrum(NUM,$("#temp_sto").val()).send({from: web3.eth.defaultAccount, gas:3000000});
  });
  ...
  function blindclick(num, temp){
    Contract.methods.storageDrum(num,temp).send({from: web3.eth.defaultAccount, gas:3000000});//데모를 위한 시간 지연  
 }
